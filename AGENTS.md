...
- Antigravity 超时不等于实现失败。超时后当前 Agent 必须先检查 `git status`、`git diff` 和受影响模块的类型错误，确认哪些修改可复用；不得原样重复委派同一个大任务，应只委派剩余的一个小子目标或直接接手完成。
- 超时后的沟通必须明确说明：worker 已完成的部分、尚未完成的部分、是否发生权限拒绝/命令失败，以及当前 Agent 的接管计划；在验证通过前不得向用户宣称需求已完成。
- Antigravity 不负责最终提交和推送，除非用户明确指定；当前 Agent 负责合并阶段性结果、处理冲突、运行完整验证、提交 Git，并更新对应 Linear 需求。
