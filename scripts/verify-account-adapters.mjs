...
try {
  ...
  put('.codex/auth.json', codexCredential)
  put('.codex/config.toml', 'forced_login_method = "api"\n')
  assert.throws(() => c.readCurrentCredential())
  put('.codex/config.toml', 'forced_chatgpt_workspace_id = "other-account"\n')
  assert.throws(() => c.readCurrentCredential())
  put('.codex/config.toml', 'cli_auth_credentials_store = "keyring"\n')
  assert.throws(() => c.readCurrentCredential())
  put('.codex/config.toml', 'cli_auth_credentials_store = "file"\n')
  assert.equal(c.readCurrentCredential(), codexCredential)
  ...
  const fullClaude = JSON.stringify({ claudeAiOauth: { accessToken: oauth, refreshToken: 'synthetic-claude-refresh', expiresAt: Date.now() + 3600_000, scopes: ['user:profile', 'user:inference'] }, oauthAccount: { accountUuid: 'user-1', organizationUuid: 'org-1', emailAddress: 'claude@example.invalid' } })
  const fullFile = put('.claude/.credentials.json', fullClaude)
  assert.equal(claude.inspect(claude.readCurrentCredential()).email, 'claude@example.invalid')
  claude.writeSlot('oauth', oauth)
  assert.equal(claude.matchesIdentity(oauth, fullClaude), true)
  assert.equal(claude.mergeCredential(oauth, fullClaude), claude.inspect(fullClaude).credential)
  assert.equal(claude.desired(fullClaude).oauth, oauth)
  writeFileSync(fullFile, '{invalid-fallback')
  assert.equal(claude.readCurrentCredential(), oauth, 'Configured token overrides unusable fallback')
  claude.writeSlot('oauth', null)
  assert.throws(() => claude.readCurrentCredential())
  rmSync(fullFile)
  ...
  const fullGoogle = JSON.stringify({ auth_method: 'consumer', token: { access_token: 'synthetic-google-access', refresh_token: 'synthetic-google-refresh', token_type: 'Bearer', expiry: new Date(Date.now() + 3600_000).toISOString() }, account: { id: 'google-1', email: 'google@example.invalid' }, oauth_client_id: 'public-client' })
  const googleProjection = adapters.antigravity.desired(fullGoogle).oauth
  assert.equal(JSON.parse(googleProjection).account, undefined)
  assert.equal(adapters.antigravity.matchesIdentity(googleProjection, fullGoogle), true)
  assert.equal(adapters.antigravity.inspect(adapters.antigravity.mergeCredential(googleProjection, fullGoogle)).email, 'google@example.invalid')
