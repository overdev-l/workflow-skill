...
4. 关闭并新建工具会话，核对账号身份；运行中的会话不会被强制退出。Antigravity 原生切换／回滚检测到客户端或 CLI 尚在运行时会拒绝写入，需先结束会话。Trace 尚未实现后台 refresh-token 自动续期；已知过期凭据会明确拒绝，可重新 OAuth 登录，或通过官方工具刷新后重新导入。
