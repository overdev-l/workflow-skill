...
入口：左侧「账号管理」。页面采用导航＋账号工作台两栏，右上角按 Claude／Codex／Antigravity 切换工具，下方以网格卡片显示账号名称、已知邮箱、当前配置与配额。命令面板也可搜索「账号管理」跳转；原「设置 → 常规与外观 → AI 账号管理」入口仍可使用，两处复用同一实现。切换只写目标工具的认证数据，不切换模型、MCP、Skill 或软链接。
