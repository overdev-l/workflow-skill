...
2026-09-16 Antigravity UI 展示修复：账号管理按当前模型选择器的 7 个可见模型收敛展示——Gemini 3.8/3.7/3.6 Flash、Gemini 3.1 Pro Low、Claude Sonnet/Opus Thinking、GPT-OSS 120B；每个模型作为一个分组，组内显示周额度与 5 小时额度，不再平铺内部模型变体或重复模型名称。
