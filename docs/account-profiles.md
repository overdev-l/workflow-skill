...
2026-09-16 Antigravity UI 展示修复：账号管理按当前可确认的 5 个模型收敛展示——Gemini 3.6 Flash Medium、Gemini 3.1 Pro Low、Claude Sonnet/Opus Thinking、GPT-OSS 120B；移除不存在的 Gemini 3.7/3.8，不再直接展示接口返回的内部模型变体。每个模型作为一个分组，组内显示周额度与 5 小时额度。
